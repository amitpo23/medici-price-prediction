...
class DynamicPricer:
    ...
    def calculate_recommended_price(
        ...
        """Calculate a recommended price with adjustments.
        ...
        if occupancy_rate is not None:
            if occupancy_rate > 0.85:
                factor = 1.0 + (occupancy_rate - 0.85) * 2  # Up to +30% at 100%
                adjustments["high_demand"] = factor
                base *= factor
            elif occupancy_rate < 0.4:
                factor = 0.85 + occupancy_rate * 0.375  # Down to -15% at 0%
        ...
        if competitor_price is not None and competitor_price > 0:
            ...
            if ratio > 1.15:
                # We're 15%+ more expensive — slight discount
                factor = 0.95
                adjustments["competitor_high"] = factor
                base *= factor
            elif ratio < 0.85:
                # We're 15%+ cheaper — slight increase
                factor = 1.05
