...
class TestDefaultWeights:
    ...
    def test_forward_curve_is_dominant(self):
        w = DeepPredictor.DEFAULT_WEIGHTS
        assert w["forward_curve"] == 0.55
        assert w["forward_curve"] > w["historical_pattern"]
        assert w["forward_curve"] > w["ml_forecast"]

    def test_historical_25_pct(self):
        assert DeepPredictor.DEFAULT_WEIGHTS["historical_pattern"] == 0.25
...
class TestComputeWeights:
    ...
    def test_weight_formula(self):
        ...
        weights = predictor._compute_weights(signals)
        # Manual calculation (weights: FC=0.55, Hist=0.25)
        raw_fc = 0.55 * (0.5 + 0.5 * 0.8)  # 0.55 * 0.90 = 0.495
        raw_hist = 0.25 * (0.5 + 0.5 * 0.6)  # 0.25 * 0.80 = 0.20
