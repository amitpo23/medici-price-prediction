...
> **Scan run:** 2026-05-27 08:00 UTC | **Data from:** 2026-04-24 04:17:19 UTC | **Check-in:** 2026-06-10 → 2026-06-11 | **Refundable only**
>
> ⚠️ **Note:** Day 41 of consecutive cloud scan block — **Slot 60.** Azure SQL port 1433 TCP-blocked from cloud egress (Login timeout expired — confirmed this slot). Innstant B2B login required — WebFetch confirmed hotel page loads but returns **zero room pricing** without auth (login link visible, no offers rendered); `INNSTANT_PASS` / `INNSTANT_USER` / `INNSTANT_ACCOUNT` env vars NOT_SET. No Playwright/browser tools in this remote execution environment. **Data age: ~795h (~33.1 days).** Set `INNSTANT_PASS` env var in Remote Trigger config to immediately unblock all future scans.

---

## Executive Summary

| Metric | Value | vs May 27 00:00 | vs May 26 16:00 | vs May 26 08:00 | vs May 26 00:00 | vs May 25 16:00 | vs May 25 08:00 | vs May 25 00:00 | vs May 24 16:00 | vs May 16 | vs Apr 24 | 30-Day Trend |
|--------|-------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------|-----------|--------------|
...
### Key Insight
**Day 41, 08:00 UTC slot (Slot 60) — scan blocked. Credentials remain INVALID. Azure SQL unreachable.**
...
All metrics static — data frozen at Apr 24 04:17 UTC (**~795h / ~33.1 days** stale). Knowaa holds 3 positions as #1 cheapest at a consistent 5.66% margin vs InnstantTravel across all 3 properties. Zero price movement since ~Apr 19 — confirmed across 60 consecutive blocked scan slots.
...
| Total blocked scan slots | **60** | Slot 60 = May 27 08:00 UTC |
...
_Knowaa is the lowest refundable price. All 3 wins: Standard RO board vs InnstantTravel. Prices locked ~38 days with zero movement._

| Hotel | VenueId | Cat | Board | Knowaa $ | 2nd $ | 2nd Provider | Gap $ | Gap % | Trend |
|-------|---------|-----|-------|----------|-------|--------------|-------|-------|-------|
| citizenM Miami Brickell hotel | 5079 | Standard | RO | **$177.23** | $187.86 | InnstantTravel | -$10.63 | -5.66% | → Steady (~920h) |
| DoubleTree by Hilton Miami Doral | 5082 | Standard | RO | **$182.63** | $193.59 | InnstantTravel | -$10.96 | -5.66% | → Steady (~920h) |
| Pullman Miami Airport | 5080 | Standard | RO | **$133.45** | $141.46 | InnstantTravel | -$8.01 | -5.66% | → Steady (~920h) |
...
|--------|-----------|---------------|-----------|-----------|----------|
| **May 27 08:00 (THIS)** | **3 (7%)** | **3 (7%)** | **28 (61%)** | **15 (33%)** | **~795h stale** |
...
**Net change since last live scan: 0 hotels changed in any category. Data frozen for 33+ consecutive days.**
...
| 🟡 P1 | Verify Jun 10-11 allotment reload for 3 Section A hotels | Revenue | Static rate since Apr 19 (~38.3 days) |
...
_Report generated by Knowaa Competitive Scanner agent | Slot 60 | 2026-05-27 08:00 UTC_
