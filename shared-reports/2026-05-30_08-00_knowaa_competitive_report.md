...
> **Scan run:** 2026-05-30 08:00 UTC | **Data from:** 2026-04-24 04:17:19 UTC | **Check-in:** 2026-06-10 → 2026-06-11 | **Refundable only**
>
> ⚠️ **Note:** Day 44, Slot 69 of consecutive cloud scan block. TCP probe to Azure SQL port 1433 returns ECONNREFUSED (error code 11) — egress blocked at network level; `INNSTANT_PASS` / `INNSTANT_USER` / `INNSTANT_ACCOUNT` env vars NOT_SET. WebFetch confirms Innstant B2B login page loads (HTTP 200, Copyright 2026) but returns **zero room pricing** without auth. **Data age: ~868h (~36.2 days).** ✅ Infrastructure ready: pyodbc module + unixODBC + ODBC Driver 18 for SQL Server all confirmed installed — TCP 1433 egress is the sole remaining DB blocker. Set `INNSTANT_PASS`/`INNSTANT_USER`/`INNSTANT_ACCOUNT` env vars AND open TCP 1433 egress in Remote Trigger config to immediately unblock all future scans.

---

## Executive Summary

| Metric | Value | vs May 30 00:00 | vs May 29 16:00 | vs May 29 08:00 | vs May 29 00:00 | vs May 28 16:00 | vs May 28 08:00 | vs May 28 00:00 | vs May 27 16:00 | vs May 27 08:00 | vs May 27 00:00 | vs May 26 16:00 | vs May 25 16:00 | vs May 16 | vs Apr 24 | 30-Day Trend |
|--------|-------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------|-----------|--------------|
...
### Key Insight
**Day 44, 08:00 UTC slot (Slot 69) — scan blocked. Credentials remain INVALID. Azure SQL TCP-blocked.**

TCP probe to `medici-sql-server.database.windows.net:1433` returns error code 11 (ECONNREFUSED) — confirmed blocked at network egress level. All three data paths remain closed: (1) Azure SQL via pyodbc — connection refused (TCP port 1433 blocked; **pyodbc + unixODBC + ODBC Driver 18 all installed** — this path activates the moment TCP 1433 egress is opened); (2) WebFetch to `b2b.innstant.travel` — login page loads (HTTP 200, Copyright 2026) but renders **zero room offers** without valid session cookies; (3) No Playwright/browser tools in this remote execution environment.

All metrics static — data frozen at Apr 24 04:17 UTC (**~868h / ~36.2 days** stale). Knowaa holds 3 #1 positions at a consistent 5.66% margin vs InnstantTravel across all 3 properties. Zero price movement since ~Apr 19 — confirmed across 69 consecutive blocked scan slots.
...
| Azure SQL port 1433 | 🔴 BLOCKED | TCP ECONNREFUSED (error 11) — egress blocked at network level |
...
| Consecutive blocked days | **44** | Apr 17 → May 30 |
| Total blocked scan slots | **69** | Slot 69 = May 30 08:00 UTC |
...
_Knowaa is the lowest refundable price. All 3 wins: Standard RO board vs InnstantTravel. Prices locked ~41 days with zero movement._

| Hotel | VenueId | Cat | Board | Knowaa $ | 2nd $ | 2nd Provider | Gap $ | Gap % | Trend |
|-------|---------|-----|-------|----------|-------|--------------|-------|-------|-------|
| citizenM Miami Brickell hotel | 5079 | Standard | RO | **$177.23** | $187.86 | InnstantTravel | -$10.63 | -5.66% | → Steady (~992h) |
| DoubleTree by Hilton Miami Doral | 5082 | Standard | RO | **$182.63** | $193.59 | InnstantTravel | -$10.96 | -5.66% | → Steady (~992h) |
| Pullman Miami Airport | 5080 | Standard | RO | **$133.45** | $141.46 | InnstantTravel | -$8.01 | -5.66% | → Steady (~992h) |
...
|--------|-----------|---------------|-----------|-----------|----------|
| **May 30 08:00 (THIS)** | **3 (7%)** | **3 (7%)** | **28 (61%)** | **15 (33%)** | **~868h stale** |
...
**Net change since last live scan: 0 hotels changed in any category. Data frozen for ~36.2 consecutive days.**
...
| 🟡 P1 | Verify Jun 10-11 allotment reload for 3 Section A hotels | Revenue | Static rate since Apr 19 (~41+ days) |
...
_Report generated by Knowaa Competitive Scanner agent | Slot 69 | 2026-05-30 08:00 UTC_
