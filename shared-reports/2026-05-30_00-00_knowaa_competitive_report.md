...
> **Scan run:** 2026-05-30 00:00 UTC | **Data from:** 2026-04-24 04:17:19 UTC | **Check-in:** 2026-06-10 → 2026-06-11 | **Refundable only**
>
> ⚠️ **Note:** Day 44, Slot 68 of consecutive cloud scan block. TCP probe to Azure SQL port 1433 returns ECONNREFUSED (error code 11) — egress blocked at network level; `INNSTANT_PASS` / `INNSTANT_USER` / `INNSTANT_ACCOUNT` env vars NOT_SET. WebFetch confirms Innstant B2B login page loads (HTTP 200, Copyright 2026) but returns **zero room pricing** without auth. **Data age: ~860h (~35.8 days).** ✅ Infrastructure ready: pyodbc module + unixODBC all confirmed installed — TCP 1433 egress is the sole remaining DB blocker. Set `INNSTANT_PASS`/`INNSTANT_USER`/`INNSTANT_ACCOUNT` env vars AND open TCP 1433 egress in Remote Trigger config to immediately unblock all future scans.

---

## Executive Summary

| Metric | Value | vs May 29 16:00 | vs May 29 08:00 | vs May 29 00:00 | vs May 28 16:00 | vs May 28 08:00 | vs May 28 00:00 | vs May 27 16:00 | vs May 27 08:00 | vs May 27 00:00 | vs May 26 16:00 | vs May 25 16:00 | vs May 16 | vs Apr 24 | 30-Day Trend |
|--------|-------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------------|-----------|-----------|--------------|
...
### Key Insight
**Day 44, 00:00 UTC slot (Slot 68) — scan blocked. Credentials remain INVALID. Azure SQL TCP-blocked.**

TCP probe to `medici-sql-server.database.windows.net:1433` returns error code 11 (ECONNREFUSED) — confirmed blocked at network egress level. All three data paths remain closed: (1) Azure SQL via pymssql/pyodbc — connection refused (TCP port 1433 blocked; **pyodbc + unixODBC all installed** — this path activates the moment TCP 1433 egress is opened); (2) WebFetch to `b2b.innstant.travel` — login page loads (HTTP 200, Copyright 2026) but renders **zero room offers** without valid session cookies; (3) No Playwright/browser tools in this remote execution environment.

All metrics static — data frozen at Apr 24 04:17 UTC (**~860h / ~35.8 days** stale). Knowaa holds 3 #1 positions at a consistent 5.66% margin vs InnstantTravel across all 3 properties. Zero price movement since ~Apr 19 — confirmed across 68 consecutive blocked scan slots.
...
| Total blocked scan slots | **68** | Slot 68 = May 30 00:00 UTC |
...
|-------|---------|-----|-------|----------|-------|--------------|-------|-------|-------|
| citizenM Miami Brickell hotel | 5079 | Standard | RO | **$177.23** | $187.86 | InnstantTravel | -$10.63 | -5.66% | → Steady (~984h) |
| DoubleTree by Hilton Miami Doral | 5082 | Standard | RO | **$182.63** | $193.59 | InnstantTravel | -$10.96 | -5.66% | → Steady (~984h) |
| Pullman Miami Airport | 5080 | Standard | RO | **$133.45** | $141.46 | InnstantTravel | -$8.01 | -5.66% | → Steady (~984h) |
...
|--------|-----------|---------------|-----------|-----------|----------|
| **May 30 00:00 (THIS)** | **3 (7%)** | **3 (7%)** | **28 (61%)** | **15 (33%)** | **~860h stale** |
...
**Net change since last live scan: 0 hotels changed in any category. Data frozen for ~35.8 consecutive days.**
...
| 🔴 P0 | Open TCP 1433 egress in cloud container OR use Azure SQL HTTP proxy | Infra | TCP ECONNREFUSED confirmed (error 11) — pyodbc + pymssql + unixODBC all installed ✅ — only TCP egress is blocked |
...
_Report generated by Knowaa Competitive Scanner agent | Slot 68 | 2026-05-30 00:00 UTC_
