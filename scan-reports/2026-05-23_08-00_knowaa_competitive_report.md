...
> **Scan run:** 2026-05-23 08:00 UTC | **Data from:** 2026-04-24 04:17:19 UTC | **Check-in:** 2026-06-10 → 2026-06-11 | **Refundable only**
>
> ⚠️ **Note:** Day 36 of consecutive cloud scan block — **Slot 48.** Playwright chromium v1208 functional, login page reached (HTTP 200, SSL bypass required — `ignore_https_errors=True`, persisting), credentials confirmed INVALID (`INNSTANT_PASS` / `INNSTANT_USER` / `INNSTANT_ACCOUNT` env vars NOT_SET; hardcoded fallback `Knowaa/Amit/porat10` rejected — `/agent/login-execute` returned `/agent/login`). Azure SQL port 1433 TCP-blocked (connection timeout, errno=11). **Data age: ~700h (~29.2 days).** Provide updated `INNSTANT_PASS` to immediately unblock.

---

## Executive Summary

| Metric | Value | vs May 23 00:00 | vs May 22 16:00 | vs May 22 08:00 | vs May 21 16:00 | vs May 16 | vs Apr 24 | 30-Day Trend |
|--------|-------|-----------------|-----------------|-----------------|-----------------|-----------|-----------|--------------|
| Hotels scanned | **46** | 0 | 0 | 0 | 0 | 0 | 0 | Flat |
| Knowaa appears | **3 (7%)** | 0 (steady) | 0 (steady) | 0 (steady) | 0 (steady) | 0 (steady) | 0 (steady) | Flat |
| Knowaa #1 (cheapest) | **3 (7%)** | 0 (steady) | 0 (steady) | 0 (steady) | 0 (steady) | 0 (steady) | 0 (steady) | Flat |
| Knowaa #2 | **0 (0%)** | 0 | 0 | 0 | 0 | 0 | 0 | Flat |
| Knowaa #3+ | **0 (0%)** | 0 | 0 | 0 | 0 | 0 | 0 | Flat |
| No Knowaa (has offers) | **28 (61%)** | 0 (steady) | 0 (steady) | 0 (steady) | 0 (steady) | 0 (steady) | 0 (steady) | Flat |
| No refundable offers | **15 (33%)** | 0 (steady) | 0 (steady) | 0 (steady) | 0 (steady) | 0 (steady) | 0 (steady) | Flat |

### Key Insight
**Day 36, 08:00 UTC slot (Slot 48) — scan blocked. Credentials remain INVALID.**

Playwright (chromium v1208 / 145.0.7632.6) launched successfully, reached Innstant B2B login at `/agent/login` (HTTP 200, SSL bypass `ignore_https_errors=True` required — `ERR_CERT_AUTHORITY_INVALID` persisting), form detected (AccountName / Username / Password fields confirmed visible), values filled (`AccountName=Knowaa`, `Username=Amit`, `Password` set via native setter bypass — `readOnly=true` still present on password field), form submitted to `/agent/login-execute` — Innstant redirected back to `/agent/login` (rejected). No env vars set (`INNSTANT_PASS`, `INNSTANT_USER`, `INNSTANT_ACCOUNT` all NOT_SET). Azure SQL port 1433 TCP-blocked (errno=11, timeout from cloud egress).

All metrics static — data frozen at Apr 24 04:17 UTC (**~700h / ~29.2 days** stale). Knowaa holds 3 positions as #1 cheapest at 5.66% margin vs InnstantTravel. Zero price movement in **~825h** since ~Apr 19 — confirmed across 48 consecutive blocked scan slots.
...
- Price stability: **$0.00 movement over ~825h** (Apr 19 → May 23 08:00, ~34.4 days)

### Infrastructure Status (May 23 08:00 UTC)

| Component | Status | Notes |
|-----------|--------|-------|
| Azure SQL port 1433 | 🔴 BLOCKED | TCP timeout from cloud (errno=11) — Day 36 |
...
| Playwright chromium v1208 | ✅ INSTALLED | Browser launches (145.0.7632.6), login page loads correctly |
...
| Consecutive blocked days | **36** | Apr 18 → May 23 |
| Total blocked scan slots | **48** | Slot 48 = May 23 08:00 UTC |
...
_Knowaa is the lowest refundable price. All wins on Standard RO board vs InnstantTravel. Prices locked ~34.4 days / ~825h with zero movement._

| Hotel | VenueId | Cat | Board | Knowaa $ | 2nd $ | 2nd Provider | Gap $ | Gap % | Trend |
|-------|---------|-----|-------|----------|-------|-------------|-------|-------|-------|
| citizenM Miami Brickell hotel | 5079 | Standard | RO | **$177.23** | $187.86 | InnstantTravel | -$10.63 | -5.66% | → Steady (~825h) |
| DoubleTree by Hilton Miami Doral | 5082 | Standard | RO | **$182.63** | $193.59 | InnstantTravel | -$10.96 | -5.66% | → Steady (~825h) |
| Pullman Miami Airport | 5080 | Standard | RO | **$133.45** | $141.46 | InnstantTravel | -$8.01 | -5.66% | → Steady (~825h) |
...
_Knowaa inventory not loaded for June 10–11. InnstantTravel dominates 26 of 28 hotels; goglobal leads at 2; HyperGuestDirect⇄ at 2 (direct channel). All absences 48+ consecutive days._
...
_All 15 now at Day 48+ consecutive — escalation critically overdue. Credentials required to verify if any have recovered._

| Hotel | VenueId | Days Absent | Status |
|-------|---------|------------|--------|
| Atwell Suites Miami Brickell | 5101 | 48+ | **🔴 CRITICAL — contract gap confirmed** |
| Hilton Cabana Miami Beach | 5115 | 48+ | 🔴 CRITICAL |
| Hilton Garden Inn Miami South Beach | 5279 | 48+ | 🔴 CRITICAL |
| Hotel Chelsea | 5064 | 48+ | 🔴 CRITICAL |
| Hotel Croydon | 5131 | 48+ | **🔴 CRITICAL — contract gap confirmed** |
| Hyatt Centric South Beach Miami (City View) | 5097 | 48+ | 🔴 CRITICAL |
| InterContinental Miami | 5276 | 48+ | **🔴 CRITICAL — contract gap confirmed** |
| Kimpton Angler's Hotel | 5136 | 48+ | 🔴 CRITICAL |
| Kimpton Hotel Palomar South Beach | 5116 | 48+ | 🔴 CRITICAL |
| Metropole South Beach | 5141 | 48+ | 🔴 CRITICAL |
| SERENA Hotel Aventura Miami, Tapestry Collection by Hilton | 5139 | 48+ | 🔴 CRITICAL |
| Sole Miami, A Noble House Resort | 5104 | 48+ | 🔴 CRITICAL |
| The Albion Hotel | 5117 | 48+ | 🔴 CRITICAL |
| The Catalina Hotel & Beach Club | 5277 | 48+ | **🔴 CRITICAL — contract gap confirmed** |
| The Grayson Hotel Miami Downtown | 5094 | 48+ | 🔴 CRITICAL |

---

## Trend — 36-Day Rolling View (Jun 10–11 Window)
...
### Section A — Price Lock (~34.4 days / ~825h)

| Hotel | Apr 22 | Apr 24 04:17 | May 6 | May 9 | May 11 | May 15 | May 23 00:00 | **May 23 08:00** | Total Movement |
|-------|--------|-------------|-------|-------|--------|--------|--------------|-----------------|----------------|
| citizenM Miami Brickell (5079) | $177.23 | $177.23 | $177.23 | $177.23 | $177.23 | $177.23 | $177.23 | **$177.23** | **$0.00 (~825h)** |
| Pullman Miami Airport (5080) | $133.45 | $133.45 | $133.45 | $133.45 | $133.45 | $133.45 | $133.45 | **$133.45** | **$0.00 (~825h)** |
| DoubleTree Miami Doral (5082) | $182.63 | $182.63 | $182.63 | $182.63 | $182.63 | $182.63 | $182.63 | **$182.63** | **$0.00 (~825h)** |
...
|----------|---------------|---------|-------|--------|------|
| 🔴 **INFRA #1** | Innstant B2B password | — | **`porat10` CONFIRMED INVALID** — rejected Slot 48 (May 23 08:00 UTC). Browser functional, login form fills (native setter bypasses `readOnly=true` on password field), form submits to `/agent/login-execute`. Password is the **sole remaining blocker** | **Update `INNSTANT_PASS` in remote trigger env AND update `scan_cached.js` hardcoded fallback** | 36 |
| 🔴 **INFRA #2** | Cloud SQL block | — | Azure SQL port 1433 TCP-blocked (errno=11) — Day 36 | Whitelist cloud egress IP or use DB proxy | 36 |
| ⚠️ **INFRA #3** | SSL certificate | — | `ERR_CERT_AUTHORITY_INVALID` — persisting; `ignore_https_errors=True` bypass succeeds | Verify SSL cert validity on Innstant B2B | Day 36 |
| 🔴 CRITICAL | Atwell Suites, Croydon, InterContinental, Catalina | 5101, 5131, 5276, 5277 | 48+ days Section E — contract gap confirmed | Contact contracting team NOW | 48+ |
| 🔴 CRITICAL | All other Section E (11 hotels) | 5064, 5094, 5097, 5104, 5115, 5116, 5117, 5136, 5139, 5141, 5279 | 48+ days no refundable offers | Escalate to contracting | 48+ |
| 🔴 URGENT | Embassy Suites (5081), Hotel Riu Plaza (5109) | 5081, 5109 | Was Knowaa #1 in May window; absent June for 48+ days | Load June allotment | 48+ |
| 🔴 HIGH | Notebook Miami Beach | 5102 | Cheapest at $65.07 — zero Knowaa | Investigate contract | 48+ |
| 🔴 HIGH | HOLIDAY INN EXPRESS | 5130 | $114.92 solo — easy Knowaa win | Load June inventory | 48+ |
| 🟡 MED | Pod Times Square, Viajero Miami | 5305, 5111 | HyperGuestDirect⇄ direct channel — no Knowaa | Evaluate rate parity | 48+ |
...
| Consecutive blocked days | **36** (since Apr 18) |
| Total blocked slots | **48** (Slot 48 = May 23 08:00 UTC) |
| pyodbc | ✅ 5.3.0 installed; TCP blocked at network level (errno=11) |
| unixodbc | ✅ 2.3.12 installed |
| msodbcsql18 | ✅ 18.6.2.1 installed; TCP to port 1433 blocked |
| Playwright chromium v1208 | ✅ INSTALLED — browser 145.0.7632.6 launches |
...
_Generated by Knowaa Competitive Scanner Agent — 2026-05-23 08:00 UTC_
